...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private Vector3 _flightVelocity; // текущая скорость полета
        ...
        [Header("Flight Mode")] public float FlightDelay = 0.5f; // Задержка активации режима полета
        private float _flightDelayTimer;

        public float FlightSprintSpeed = 10f;
        ...
        private void Move()
        {
            // Проверка на нажатие кнопки прыжка для включения/выключения режима полёта
            if (_input.jump)
            {
            ...
            if (IsFlying)
            {
                // Включаем движение по Y в полёте, чтобы персонаж двигался вверх/вниз в зависимости от камеры
                ...
                _flightVelocity = Vector3.Lerp(_flightVelocity, targetVelocity, Time.deltaTime * FlightInertia);

                // Движение персонажа в том числе по оси Y
                _controller.Move(_flightVelocity * Time.deltaTime);
            }
            else
            {
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                // Обновляем анимацию, если используется
                if (_hasAnimator)
                {
                    _animator.SetBool(_animIDJump, false);
                    _animator.SetBool(_animIDFreeFall, false); // Сбрасываем анимацию падения, если персонаж на земле
                }

                // Останавливаем вертикальную скорость при касании земли
                ...
                {
                    // Вычисляем нужную скорость для прыжка
            ...
            else
            {
                ...
                {
                    // Если персонаж не на земле и не прыгает, включаем анимацию падения
                    if (_verticalVelocity < 0 && _hasAnimator)
                    {
